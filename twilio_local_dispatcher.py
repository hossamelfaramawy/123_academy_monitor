...
def resolve_skill_id(sheet_val, subject, curriculum):
    ...
    for tag in ["an_01", "an_02", "ap_01", "ad"]:
        if tag in val_clean:
            for skill_id, skill in curriculum.items():
                if skill.get("subject") == subject:
...
def main():
    ...
    # Display configuration (with masked secrets for security)
    ...
    for row_idx, row in enumerate(all_records[1:], start=2):
        ...
        if not pending_skills:
            print(f"  ℹ️ No pending skills to send for '{student_name}'.")
            continue
        ...
        try:
            ...
            print(f"  ❌ Failed to send message / update sheet cells: {err}")
            fail_count += 1

    print("\n-------------------------------------------------------")
    print(f"🏁 LOCAL Dispatch Completed at {current_time_str}")
